...
| FR-4.5 | Objection-clearing resume rewrite | ✅ | `generateSophisticatedResume()` `apps/api/src/resume/engine.ts` — V1 32-role prompts/templates (`roleRegistry.ts`), ATS scorer (`ats.ts`), moderation loop (`moderator.ts`, max 2 iterations), committee GAP report + transcript injected into the system prompt; US/UK English variant from job location (`locale.ts`); `[ADD: ...]` placeholders for missing evidence | E2E rewritten-resume Markdown + JSON + meta produced; unit tests `resume/engine.test.ts`, `resume/ats.test.ts`, `resume/roleRegistry.test.ts`, `resume/serialize.test.ts`, `resume/locale.test.ts` |
