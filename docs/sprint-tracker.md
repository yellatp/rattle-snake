...
### Implemented (Part A5 — sophisticated role-targeted resume engine from V1)
- **Port of Rattle-Snake V1's optimised resume system** into V2 (`apps/api/src/resume/`): all **32 role prompts** (`prompts/*.ts`, copied verbatim, `qa`/`cover_letter` excluded) + **32 role templates** (`templates/*.ts`, BOM-stripped JSON wrappers), the **ATS scorer** (`ats.ts`, fuse.js dependency replaced with a normalized Levenshtein fuzzy matcher), the **quality moderator** (`moderator.ts`, LLM-based with banned-phrase checks), `json.ts` (sanitize + extract resume JSON), and `serialize.ts` (single-source-of-truth `resumeToMarkdown`).
- **Role detection** (`roleRegistry.ts`) — `resolveRoleSlug(domain, jd)`: (1) explicit JD **title signal** (e.g. "backend engineer" → `swe`, "cloud engineer" → `cloud_engineer`) scored per domain; (2) **keyword overlap** of each domain candidate's `ats_keywords` against the JD; (3) domain **flagship fallback**. Fixes the bug where a "Senior Backend Engineer" JD was mis-routed to `data_engineer`.
- **Engine** (`engine.ts`) — `generateSophisticatedResume(job, blueprint, llm)`: gap analysis on the base resume → role detection → system prompt = role prompt + `## HIRING COMMITTEE FEEDBACK` (GAP report + up to 8 transcript excerpts) → V1-style user prompt (Role/JD/Source Resume **as-is**/Template skeleton/Tone/matched+missing keywords) → `llm.complete` (temp 0.3, max 4000) → JSON extract → ATS re-score → moderation loop (**max 2 iterations**; regenerates only on moderation rejection, injecting `## MODERATOR FEEDBACK`; unparseable JSON → synthetic score 30 → regenerate) → markdown. Outputs `{ markdown, json, meta }`. Candidate is kept "as-is" — no filler, invented facts become `[ADD: …]` placeholders.
- **Storage** — `JobState.rewrittenResumeJson` + `resumeMeta` (role, roleLabel, atsScore, matched/missing keywords, moderationScore, moderationApproved, iterations) with `rewritten_resume_json`/`resume_meta` columns + safe ALTER migrations.
- **API** — job events now carry the JSON + meta; new **PUT `/api/jobs/:id/resume`** persists hand-edited resume JSON and re-renders the markdown server-side (single source of truth); `updateJobResume` in the web client.
- **UI** (`DebateView.tsx`) — rewritten resume is now a **Markdown | JSON** tabbed view; the JSON tab has an editable textarea + Save; a **`ResumeMetaBadge`** shows role, ATS %, auditor score, and pass/iteration count.
- **Mock LLM** — `MOCK_RESUME_JSON` (Rohan Mehta SWE resume) + `MOCK_MODERATION_JSON`; branches keyed on "senior resume writer" / "resume quality auditor" prompts. Old committee `resumeRewriter.ts` deleted.
- **Tests (30 new)** — `ats.test.ts` (keyword extraction/stop-words/exact+fuzzy+missing scoring), `roleRegistry.test.ts` (32 registrations, domain lists, title-signal + keyword + flagship fallback detection), `serialize.test.ts` (full render/empty-name/empty-skeleton), `engine.test.ts` (stub LLM: happy path iterations=1, committee feedback in system prompt, reject→approve regeneration with MODERATOR FEEDBACK, unparseable-JSON regeneration, iteration cap), + 3 jobs route tests (structured JSON+meta, PUT resume edit re-renders markdown, invalid JSON → 400). Suite now **112 tests** (13 shared + 99 api); build 3/3; typecheck 4/4; `pnpm e2e` PASSED with new resume-meta assertions.

### Implemented (Part A5.1 — US/UK English variant from job location)
- **The generated resume now matches the job's country language conventions** — a must-have requested during the resume-engine review.
- `apps/api/src/resume/locale.ts` — `detectEnglishLocale(jobLocation, jobDescription)` (explicit `location` field wins, then UK/US markers in the JD, else US default) + `buildEnglishVariantDirective()` — a **standardised block** injected into the role system prompt spelling out spelling (-ize/-yze/-or/single-L vs -ise/-yse/-our/double-L), terminology (resume/cell vs CV/mobile, $/USD vs £/GBP), date formats, and a "no mixed variants" rule.
- **API/UI** — new optional `location` on `POST /api/jobs` (zod `location`), persisted to `JobState.jobLocation` via a `job_location` column + safe ALTER migration; "Job location" input on the New Debate form (auto-detect from JD when blank); `resumeMeta.locale` ("us"|"uk") surfaced in the resume meta badge ("English: US/UK").
- **Moderator** — the quality auditor now enforces the variant: its prompt gains a §6 ENGLISH VARIANT CHECK + a per-locale note flagging words from the wrong variant (score −5 each).
- **Tests (12 new)** — `locale.test.ts` (10: default, UK/US location field, JD markers, field-overrides-JD, directive content), 2 engine tests (US directive for a US job, UK directive + moderator note for a UK job), 1 jobs route test (location persisted + `locale: "uk"`); suite now **124 tests** (13 shared + 111 api). `pnpm e2e` asserts UK `locale=uk` on the wire-format path and `loc=New York, USA` + `locale=us` on the HTTP API path.
- End-to-end chain verified end-to-end: **candidate base resume + JD → committee debate judges the existing profile → `extractBlueprint` GAP report (objections/strengths/required changes) → standardised role system prompt + English-variant directive + GAP report/expert discussion → role-template JSON → ATS re-score → moderation loop → Markdown + meta**.
