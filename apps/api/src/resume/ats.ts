/**
 * ATS keyword scorer — ported from the Rattle-Snake V1 generator.
 *
 * Extracts high-signal terms from the job description (single tokens +
 * bigrams, frequency-weighted), then checks each against the resume using
 * exact, stemmed, and fuzzy (normalized Levenshtein) matching. The matched /
 * missing keyword lists feed the resume prompt so the generator embeds the
 * exact terms the ATS will scan for.
 */
...
/** Iterative Levenshtein distance (portable, no native deps). */
...
/**
 * Extract the same keyword list the scorer will check — use this to pre-seed
 * the AI prompt so the model embeds the exact terms that will be scored.
 */
...
export function scoreResume(
  ...
  for (const kw of importantJdKeywords) {
    ...
    const stemMatchFound =
      !exactMatch && (resumeStemmed.has(stem(kw)) || resumeStemmed.has(kw));

    // Fuzzy: closest resume term within a 0.25 normalized edit distance.
    let fuzzyMatch = false;
    if (!exactMatch && !stemMatchFound) {
      for (const term of resumeTermsArr) {
        const distance = levenshtein(kw, term);
        const norm = distance / Math.max(kw.length, term.length);
        if (norm < 0.25) {
          fuzzyMatch = true;
          break;
        }
...
/**
 * Flatten resume content to plain text for keyword scoring. Accepts either the
 * structured role-template JSON or raw text (V2 base resumes are Markdown).
 */
export function resumeToText(resumeContent: string): string {
  try {
    const parsed = JSON.parse(resumeContent) as {
      contact?: { name?: string; location?: string };
      sections?: {
    ...
    parsed.sections?.summary?.content && parts.push(parsed.sections.summary.content);
    ...
    parsed.sections?.coreCompetencies?.forEach((c) => parts.push(c));
