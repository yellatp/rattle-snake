 ...
 *
 * Point baseURL at Ollama, vLLM, LocalAI, llama.cpp, LM Studio — any server
 * exposing an OpenAI-compatible /v1 API — without changing any other code.
 */
export function createOpenAIClient(config: AppConfig["llm"]): LLMClient {
  ...
  return {
    ...
    async complete(system, user, opts) {
      ...
      if (!text) throw new Error(`Empty LLM response for model ${config.model}`);
      return text;
    },
  };
}
...
/**
 * Offline mock client. Returns deterministic, correctly-formatted responses so
 * the whole pipeline (non-neutrality enforcer, blueprint extractor, rewriter)
 * can be exercised end-to-end without any LLM running.
 */
