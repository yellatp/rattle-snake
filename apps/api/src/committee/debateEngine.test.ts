...
import type { AgentConfig, Decision, JobState, TranscriptEntry } from "@rattlesnake/shared";
...
function entry(sender: string, decision: Decision): TranscriptEntry {
  return {
...
describe("runDebate — full pipeline with mock LLM", () => {
  it("runs openings → cross-talk → ballot and produces 20 entries, all decisive", async () => {
    ...
    expect(result.entries).toHaveLength(5 + 2 * 5 + 5); // 5 opening + 10 cross-talk + 5 ballot
    ...
    expect(crosstalk).toHaveLength(10);
  ...
  it("alternates agent order between cross-talk passes", async () => {
    ...
    const firstPass = rounds.slice(0, 5).map((e) => e.sender);
    const secondPass = rounds.slice(5, 10).map((e) => e.sender);
